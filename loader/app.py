...
def create_playList(listName, listItem, uuid):
    """
    Create a song.
    If a record already exists with the same artist and title,
    the old UUID is replaced with this one.
...
if __name__ == '__main__':
    ...
    with open('{}/playlist/playlist.csv'.format(resource_dir), 'r') as inp:
        rdr = csv.reader(inp)
        next(rdr)  # Skip header
        for listname, playlist, uuid in rdr:
            resp = create_playList(listname.strip(),
                               playlist.strip(),
                               uuid.strip())
            resp = check_resp(resp, 'playlist_id')
            if resp is None or resp != uuid:
                print('Error creating song {} {}, {}'.format(listname,
                                                             playlist,
                                                             uuid))
